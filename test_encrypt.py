...
@pytest.mark.parametrize(
    ...
    (
        ...
        (7.2, [1]),
        (7.3, [1, 2]),
        (7.4, ['complex list', 1, 3.5, False, None, {'key': 'value'}, [1, 2], [1, 2]]),
        (7.5, ['complex list', 1, 3.5, False, None, {'key', 'value'}, [1, 2], (1, 2)]),
        (8.1, ()),
        (8.2, (1,)),
        (8.3, (1, 2)),
        (8.4, ('complex tuple', 1, 3.5, False, None, {'key': 'value'}, [1, 2], [1, 2])),
        (8.5, ('complex tuple', 1, 3.5, False, None, {'key', 'value'}, [1, 2], (1, 2))),
        (9.1, set()),
        (9.2, {1}),
        (9.4, {'complex set', 1, 3.5, False, None}),
...
def test__encrypt_decrypt_from_file__no_change(index: float, value: str) -> None:
    ...
    value_filename = testregression.make_filename(index=index, extension='.bin')
...
def test__encrypt_decrypt_from_file__no_change__auto_rotate(
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        secondary_encryptor.encrypt_to_file(value, value_filename, overwrite=True)
